...
angular.module('mainApp').controller('RoleDetailController',RoleDetailController);
RoleDetailController.$inject = ['$scope','$location','authData','$stateParams','resource','inform']
function RoleDetailController($scope,$location,authData,$stateParams,resource,inform){
	//返回上一页
	$scope.back = function(){
		history.back(-1);
	}
	...
	var init = function(){
		authData.roleDetail(name,namespace).then(function(data){
			$scope.role = data;
			initList();
		});
	...
	$scope.refresh = function(){
		...
		authData.putRole($scope.role.name,$scope.role.namespace,resource,noneResource).then(function(data){
			if(data.success){
				var inf = {
					title: 'Success',
