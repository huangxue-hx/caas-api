...
angular.module('mainApp').controller('CRoleNewController',CRoleNewController);
CRoleNewController.$inject = ['$scope','namespace','resource','authData','$location'];
function CRoleNewController($scope,namespace,resource,authData,$location){
	$scope.name="";
	$scope.resource = [];
	$scope.noneResource = [];
	$scope.type = 'crole';
	$scope.addText = 'newCrole';
	...
	$scope.save = function(){
		...
		authData.addCRole(sendData).then(function(data){
			if(data.success){
