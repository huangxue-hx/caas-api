...
@WebAppConfiguration
public class TenantControllerNewTest {

    private Logger LOGGER= LoggerFactory.getLogger(TenantControllerNewTest.class);
    ...
    public void createClusterQuota() throws Exception {
        ...
        String result = mockMvc.perform(put(url).contentType(MediaType.APPLICATION_FORM_URLENCODED)
                ...
                .param("clusterQuota[0].storageQuota[0].storageQuota","4")
                .param("clusterQuota[0].storageQuota[0].totalStorage","4")
                .param("clusterQuota[0].storageQuota[1].name","xc-test2")
                .param("clusterQuota[0].storageQuota[1].storageQuota","4")
                .param("clusterQuota[0].storageQuota[1].totalStorage","4")
                .accept(MediaType.APPLICATION_JSON)).andDo(print()).andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
        Map resultMap = JsonUtil.convertJsonToMap(result);
        assertTrue((boolean)resultMap.get("success"));
